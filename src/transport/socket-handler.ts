...
export function initTransport({ io, bus, rooms, agents, tools: _tools, ai }: InitTransportParams): void {
  io.on('connection', (socket: Socket) => {
    ...
    handle(socket, 'building:delete', BuildingDeleteSchema, (parsed, ack) => {
      ...
      // Cascading delete — complete FK-safe order for ALL tables.
      // Only removes from DB. Does NOT delete any files on disk.
      const txn = db.transaction(() => {
        const aq = `SELECT id FROM agents WHERE building_id = ?`;
        const rq = `SELECT r.id FROM rooms r JOIN floors f ON r.floor_id = f.id WHERE f.building_id = ?`;
        const tq = `SELECT id FROM tasks WHERE building_id = ?`;
        const fq = `SELECT id FROM floors WHERE building_id = ?`;

        // ── Phase 1: Null out all cross-references to break FK cycles ──
        db.prepare(`UPDATE agents SET current_room_id = NULL, current_table_id = NULL WHERE building_id = ?`).run(buildingId);
        db.prepare(`UPDATE tasks SET assignee_id = NULL, table_id = NULL, parent_id = NULL WHERE building_id = ?`).run(buildingId);
        ...
        db.prepare(`DELETE FROM agent_stats WHERE agent_id IN (${aq})`).run(buildingId);
        db.prepare(`DELETE FROM agent_sessions WHERE agent_id IN (${aq})`).run(buildingId);
        db.prepare(`DELETE FROM agent_activity_log WHERE building_id = ?`).run(buildingId);
        db.prepare(`DELETE FROM agent_activity_log WHERE agent_id IN (${aq})`).run(buildingId);
        ...
        // ── Phase 3: Tables that reference agents/rooms/tasks ──
        ...
        db.prepare(`DELETE FROM todos WHERE task_id IN (${tq})`).run(buildingId);
        db.prepare(`DELETE FROM todos WHERE agent_id IN (${aq})`).run(buildingId);
        db.prepare(`DELETE FROM todos WHERE room_id IN (${rq})`).run(buildingId);
    ...
    handle(socket, 'building:start', BuildingStartSchema, (parsed, ack) => {
      ...
      if (result.ok) {
        ...
        const stratRoom = db.prepare(`
          ...
          WHERE f.building_id = ? AND r.type = 'strategist'
          LIMIT 1
        `).get(parsed.buildingId) as { id: string } | undefined;

        if (stratRoom) {
          const stratAgent = db.prepare(`
            SELECT id FROM agents WHERE building_id = ? AND current_room_id = ?
            LIMIT 1
...
Focus on being helpful to a non-technical project owner. Use plain language.`,
    ...
    handle(socket, 'agent:move', AgentMoveSchema, (parsed, ack) => {
      const db = getDb();
      const agent = db.prepare('SELECT id, current_room_id, building_id FROM agents WHERE id = ?').get(parsed.agentId) as { id: string; current_room_id: string | null; building_id: string | null } | undefined;
      ...
      if (agent.current_room_id) {
        ...
        getAssociations(socket.id).roomMemberships.delete(parsed.agentId);
        bus.emit('room:agent:exited', { roomId: agent.current_room_id, agentId: parsed.agentId, reason: 'move', buildingId: agent.building_id });
      }
      ...
      if (enterResult.ok) {
        getAssociations(socket.id).roomMemberships.set(parsed.agentId, parsed.roomId);
        bus.emit('room:agent:entered', { roomId: parsed.roomId, agentId: parsed.agentId, tableType: parsed.tableType, buildingId: agent.building_id });
    ...
    handle(socket, 'agent:register', AgentRegisterSchema, (parsed, ack) => {
      ...
      if (result.ok) {
        ...
        // Broadcast to all clients viewing this building (#850)
        const registeredAgent = agents.getAgent(agentId);
        if (registeredAgent) {
          bus.emit('agent:created', { agentId, buildingId: registeredAgent.building_id, agent: registeredAgent });
        }
          ...
          generateFullProfile(ai, parsed.role, undefined, {
            ...
            if (genResult.ok) {
              ...
              if (updateResult.ok) {
                ...
                bus.emit('agent:profile-updated', { agentId, profile: updatedAgent, buildingId: updatedAgent?.building_id });
                bus.emit('agent:profile-generated', {
                  agentId,
                  generation: profileResult.generation,
                  warnings: profileResult.warnings,
                  autoGenerated: true,
                  buildingId: updatedAgent?.building_id,
                });
    ...
    handle(socket, 'agent:update-profile', AgentUpdateProfileSchema, (parsed, ack) => {
      ...
      if (result.ok) {
        ...
        bus.emit('agent:profile-updated', { agentId: parsed.agentId, profile: updatedAgent, buildingId: updatedAgent?.building_id });
    ...
    handle(socket, 'agent:update', AgentUpdateSchema, (parsed, ack) => {
      ...
      if (result.ok) {
        const updatedAgentData = agents.getAgent(agentId);
        bus.emit('agent:updated', { agentId, agent: updatedAgentData, updates: Object.keys(updates), buildingId: updatedAgentData?.building_id });
        ...
        if (assoc) {
          for (const [agentId, roomId] of assoc.roomMemberships) {
            try {
              ...
              const exitAgent = agents.getAgent(agentId);
              bus.emit('room:agent:exited', { roomId, agentId, reason: 'disconnect', buildingId: exitAgent?.building_id });
              cleanedRooms++;
            } catch (e) {
