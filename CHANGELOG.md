...
- **AI screenshot analysis** — `analyze_screenshot` tool reads PNG/JPG files and sends to AI vision API for structured analysis (UI elements, issues, description); auto-selects vision-capable provider (Anthropic/OpenAI — MiniMax M2.5 lacks vision); `setToolsAI()` dependency injection pattern; 7.5MB file size pre-check (#657, PR #659)
